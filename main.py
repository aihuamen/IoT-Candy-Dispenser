...
while True:
    face_path = img_path+'/others/capture{}.jpg'.format(str(guest_no))
...
def candy_detect_page():
    ...
    while run:
        ...
        cheese = text("Say Cheese!","Quicksand",40,X/2,Y/2)
        
        status = text("Please wait for the camera...","Quicksand",35,X/2,45)
        pygame.display.update()
        person = detect_face()
        
        if(person == 'unknown' or person in admin):
            candy_complete(person)
        else:
            candy_fail()
...
def candy_complete(person):
    ...
    if(is_admin):
        ...
        mqtt_admin(person,int(weight))
    else:
        mqtt_guest(int(weight))
        guest_no += 1
    ...
    while run:
        ...
        if(is_admin):
            status = text("Hi {}!".format(person),"Quicksand",35,X/2,45)
...
def admin_password():
    ...
    while run:
        ...
        if(cardBtn.is_clicked()):
            status = text("Opening the camera...","Quicksand",35,X/2,90)
            pygame.display.update()
            person = detect_admin()
        
            if(person in admin):
                admin_setting(person)
            else:
...
def admin_setting(name):
    ...
    while run:
        ...
        resetBtn = Button("Reset",100,50,orange,lightorange,20)
        resetBtn.place(2*X/3,Y-170)

        cardBtn = Button("Save",100,50,green,lightgreen,20)
        cardBtn.place(2*X/3,Y-100)

        if(removeBtn.is_clicked()):
            print('remove')
            if(not guest_no == 1):
                for i in range(guest_no-1):
                    os.remove('./pic/faces/others/capture{}.jpg'.format(str(i+1)))
                guest_no = 1
                
        if(resetBtn.is_clicked()):
            print('reset')
            mqtt_reset(name)
